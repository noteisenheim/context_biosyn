...
class BioSyn(object):
    ...
    def __init__(self):
        ...
        self.sent_weight = None
        # ------ MY CODE ------

    # ------ MY CODE ------
    def init_sent_encoder(self, model_type='para'):
        if model_type == 'para':
            self.sent_encoder = SentenceTransformer('paraphrase-distilroberta-base-v1')
        else:
            self.sent_encoder = SentenceTransformer('stsb-roberta-base')
        return self.sent_encoder

    def init_sent_weight(self, initial_sent_weight, use_cuda):
        if use_cuda:
            self.sent_weight = nn.Parameter(torch.empty(1).cuda())
        else:
            self.sent_weight = nn.Parameter(torch.empty(1))
        self.sent_weight.data.fill_(initial_sent_weight)
    ...
    def save_model(self, path):
        ...
        sent_weight_file = os.path.join(path,'sent_weight.pt')
        torch.save(self.sent_weight, sent_weight_file)
        logging.info("Sent weight saved in {}".format(sent_weight_file))
    ...
    def load_sent_weight(self, path):
        sent_weight_file = os.path.join(path, 'sent_weight.pt')
        self.sent_weight = torch.load(sent_weight_file)
    ...
    # ------ MY CODE ------
    def embed_sent(self, names, show_progress=False):
        """
        ...
        for start in iterations:
            ...
            batch_sent_embeds = self.sent_encoder.encode(batch)
            batch_sent_embeds = np.array(batch_sent_embeds)
            sent_embeds.append(batch_sent_embeds)
