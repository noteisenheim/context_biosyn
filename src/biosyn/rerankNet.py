...
class RerankNet(nn.Module):
    def __init__(self, encoder, learning_rate, weight_decay, sparse_weight, sent_weight, use_cuda):
        ...
        self.optimizer = optim.Adam([
            ...
            {'params': self.sent_weight, 'lr': 0.01, 'weight_decay': 0}], 
    ...
    def forward(self, x):
        ...
        output : (N, topk)
        """
        query_token, candidate_tokens, candidate_s_scores, candidate_sent_scores = x
        batch_size, topk, _ = candidate_tokens.shape
        if self.use_cuda:
            candidate_s_scores = candidate_s_scores.cuda()
            candidate_sent_scores = candidate_sent_scores.cuda()
        ...
        score = self.sparse_weight * candidate_s_scores + self.sent_weight * candidate_sent_scores + candidate_d_score
        return score
